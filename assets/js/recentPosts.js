$(document).ready(function () {
  ...
  const showRecentPosts = () => {
    const posts = JSON.parse(localStorage.getItem("posts")).slice(-6).reverse();
    ...
    $.map(posts, (post) => {
      ...
      const imgContainer = $("<p>").attr("class", "img-container");
    ...
    const linkTag = $("<a>")
      .attr("href", "http://127.0.0.1:5500/pages/all_posts.html")
      .text("View All Posts");
    const btnDiv = $("<div>").attr("class", "btn-div");

    btnDiv.append(linkTag);
    $("#recentPosts").append(btnDiv);
  };
